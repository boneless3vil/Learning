...
class PhotoMover:
    ...
    def __init__(self):
        self.model = None
        self._cleanup_model()  # Ensure clean state on startup
        self.processed = 0
    ...
    def initialize(self) -> bool:
        ...
        try:
            self._cleanup_model()  # Ensure clean state before loading

            with console.status("[bold green]Loading AI model...") as status:
                logger.info("Loading YOLOv8 model...")
                # Force CPU inference to avoid CUDA memory issues
                self.model = YOLO('yolov8n.pt', task='detect')
                self.model.cpu()  # Ensure CPU usage
    ...
    def process_images(self, source_dir: str, dest_dir: str, subject: str) -> List[str]:
        ...
        try:
            # Get list of images with batch processing
            image_files = []
            ...
            with Progress(
                ...
                batch_size = min(20, max(1, total_remaining // 100))  # Dynamic batch size
        ...
        # Only remove progress file if all processing completed successfully
